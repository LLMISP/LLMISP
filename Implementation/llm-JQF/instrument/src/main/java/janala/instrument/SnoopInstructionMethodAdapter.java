...
import edu.berkeley.cs.jqf.instrument.InstructionClass;
...
public class SnoopInstructionMethodAdapter extends MethodVisitor implements Opcodes {
  ...
  public static HashMap<InstructionClass, String> branchesMap = new HashMap<>();
...
//  }
  ...
  private void addInstNumber(int iid){
    addInstNumber(iid, 0);
  }

  private void addInstNumber(int iid, int branchTaken){
    String str = className + "#" + methodName + descriptor;
    branchesMap.put(new InstructionClass(iid, branchTaken), str);
  }
}
