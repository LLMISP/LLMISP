...
This project presents a new goal for JQF.

**CURRENT:** Now only support shell-script to use LLMFuzz.
```shell
...
```
**DEPRECATED:** I don't know how to attach javaagent to a maven plugin goal (Here is LLMGoal), so I deprecated this.

```shell
# e.g. mvn jqf:llm -Dclass="edu.berkeley.cs.jqf.examples.guava.CacheEvictionTest" -Dmethod="testLru"
mvn jqf:llm -Dclass="target-class" -Dmethod="target-method"

# if you want to use customized object input
# e.g. mvn jqf:llm -Dclass="edu.berkeley.cs.jqf.examples.guava.CacheEvictionTest" -Dmethod="testLru" -DkeepOriginObj 
mvn jqf:llm -Dclass="target-class" -Dmethod="target-method" -DkeepOriginObj

# add javaagent option into MAVEN_OPTS
MAVEN_OPTS="-javaagent:../instrument/target/jqf-instrument-2.1-SNAPSHOT.jar -classpath ../instrument/target/classes:/home/jump/.m2/repository/org/ow2/asm/asm/9.5/asm-9.5.jar" mvn jqf:llm  -Dclass="edu.berkeley.cs.jqf.examples.guava.CacheEvictionTest" -Dmethod="testLru" -DkeepOriginObj -X
```
